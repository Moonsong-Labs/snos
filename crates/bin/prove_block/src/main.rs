...
use starknet_os_types::casm_contract_class::{BlockifierCasmClass, GenericCasmContractClass};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    for contract_address in &accessed_contracts {
        if let Ok(class_hash) = provider.get_class_hash_at(BlockId::Number(block_number), contract_address).await {
            ...
            accessed_class_hashes.insert(class_hash_felt);

            // let _ = generic_sierra_cc.get_cairo_lang_contract_class()?; // forces a different serialization path...
